  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-[#1E3932] flex items-center justify-center">
          <SchoolIcon className="text-white" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-[#1E3932]">School Profile Theme</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sections.map((section) => (
          <Link
            ...
            className="group block bg-white border border-gray-200 rounded-xl p-5 hover:border-[#00704A] hover:shadow-md transition-all"
          >
            <div className="flex items-start justify-between">
              <div>
                <h2 className="font-semibold text-gray-900 group-hover:text-[#00704A] transition-colors">
                  {section.name}
                </h2>
                <p className="text-sm text-gray-500 mt-1">{section.description}</p>
              </div>
              <span className="text-gray-300 group-hover:text-[#00704A] transition-colors text-xl">›</span>
            </div>
          </Link>
        ))}
      </div>

      <div className="mt-8 p-4 bg-[#D4E9E2]/30 border border-[#00704A]/20 rounded-xl">
        <p className="text-sm text-[#1E3932]">
          <strong>Note:</strong> Changes are saved per-section. Visit the{" "}
          <Link href="/" target="_blank" className="text-[#00704A] underline">public homepage</Link>{" "}
