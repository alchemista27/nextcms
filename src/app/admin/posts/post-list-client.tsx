...
import { trashPost, restorePost, permanentDeletePost, bulkAction } from "@/actions/post";
import { formatDate } from "@/lib/utils";
import DeleteOutlineIcon from "@mui/icons-material/DeleteOutlined";
import RestoreIcon from "@mui/icons-material/Restore";
import SearchIcon from "@mui/icons-material/Search";

type PostStatus = "DRAFT" | "PUBLISHED" | "PENDING" | "TRASH";
...
export default function PostListClient({
  ...
  const handleBulkAction = (action: "publish" | "trash" | "delete") => {
    ...
    const msg =
      action === "delete"
        ? `Permanently delete ${selectedIds.length} post(s)? This cannot be undone.`
        : `${action === "publish" ? "Publish" : "Move to trash"} ${selectedIds.length} post(s)?`;
            ...
            <div className="flex items-center gap-2 text-xs opacity-0 group-hover:opacity-100 transition-opacity">
              {!isTrash ? (
                <>
                  ...
                  <span className="text-gray-300">|</span>
                  <a href={`/posts/${post.slug}`} target="_blank" className="text-gray-500 hover:underline">View</a>
                </>
              ) : (
                <>
                  <button
                  ...
                  <button
                    onClick={() => {
                      if (confirm("Delete permanently? This cannot be undone.")) {
                        startTransition(async () => { await permanentDeletePost(post.id); router.refresh(); });
                      }
                    }}
                    disabled={isPending}
        ...
        <div className="flex flex-wrap gap-1">
          {row.original.categories.length === 0 ? (
            ...
            row.original.categories.map((c) => (
              <span key={c.id} className="bg-gray-100 text-gray-600 text-xs px-1.5 py-0.5 rounded">
                {c.name}
              </span>
            ))
          )}
        ...
        <div className="flex flex-wrap gap-1">
          {row.original.tags.length === 0 ? (
            ...
            row.original.tags.slice(0, 3).map((t) => (
              <span key={t.id} className="bg-blue-50 text-blue-600 text-xs px-1.5 py-0.5 rounded">
                {t.name}
              </span>
            ))
          )}
          {row.original.tags.length > 3 && (
            <span className="text-gray-400 text-xs">+{row.original.tags.length - 3}</span>
          )}
        </div>
      ),
    },
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50/50">
            <span className="text-sm text-gray-500">
              Showing {Math.min((currentPage - 1) * 20 + 1, total)}–{Math.min(currentPage * 20, total)} of {total}
            </span>
